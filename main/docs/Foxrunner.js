...
function updateInstructionCosts(options = {}) {
    ...
    try {
        ...
        const instr = document.getElementById('instructions');
            function storeOriginalFoxColor(model) {
                if (!model) return;
                model.traverse(obj => {
                    if (obj.isMesh && obj.material) {
                        if (Array.isArray(obj.material)) {
                            obj.material.forEach(mat => {
                                if (mat.color && !obj.userData.originalColor) {
                                    obj.userData.originalColor = mat.color.clone();
                                }
                            });
                        } else if (obj.material.color && !obj.userData.originalColor) {
        ...
        if (retries > 0) {
            setTimeout(() => updateInstructionCosts({ retries: retries - 1, intervalMs, silent }), intervalMs);
                // Store original fox color for respawn restoration
                storeOriginalFoxColor(this.model);
        } else if (!silent) {
...
class CharacterController {
    ...
    try { this.game?.audioManager?.stopLavaHurtLoop?.(); } catch(e) { try { this.audioManager?.stopLavaHurtLoop?.(); } catch{} }
        ...
        this.currentSpeedMultiplier = 1.0;
            ...
            // Restore fox color on respawn
            if (this.model) {
                this.model.traverse(obj => {
                    if (obj.isMesh && obj.material) {
                        const orig = obj.userData?.originalColor;
                        if (Array.isArray(obj.material)) {
                            obj.material.forEach(mat => {
                                if (mat.color) mat.color.copy(orig || new THREE.Color(0xffa500));
                            });
                        } else if (obj.material.color) {
                            obj.material.color.copy(orig || new THREE.Color(0xffa500));
                        }
                    }
                });
            }
